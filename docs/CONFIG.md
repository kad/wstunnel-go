...
- `no_color`: currently a no-op; colored log output is not configurable yet.
...
- `mode` is used when invoking the binary with only `--config`. The current runtime does not yet apply `log_lvl` from YAML automatically; use the CLI flag for log level. `no_color` and the `--no-color` flag are currently no-ops.
...
- `connection_retry_max_backoff`: currently a no-op; reserved for capping regular client connection retries.
- `reverse_tunnel_connection_retry_max_backoff`: max backoff while retrying reverse-tunnel control connections.
- `http_proxy`: currently a no-op; reserved for configuring an upstream HTTP proxy URL used to reach the wstunnel server.
- `http_proxy_login`: currently a no-op; reserved for an optional proxy username override.
- `http_proxy_password`: currently a no-op; reserved for an optional proxy password override.
- `dns_resolver`: currently a no-op; reserved for configuring explicit DNS resolvers to use.
- `dns_resolver_prefer_ipv4`: currently a no-op; reserved for preferring IPv4 answers when both families are available.
...
- `tcp://9090:localhost:443` under `remote_to_local`: reverse TCP tunnel. The current Go implementation uses the target host/port as both the server-side listener address and the client-side dial target, so it does not yet support a separate server listener address mapped to a different local target.
...
- `websocket_mask_frame`: currently a no-op; reserved for future use and does not affect websocket frame masking.
- `socket_so_mark`: Linux `SO_MARK` value for server listener sockets, including reverse-tunnel listeners. It is not applied to outbound target dials made by the server.
- `dns_resolver`: currently a no-op; reserved for explicit resolvers the server should use.
- `dns_resolver_prefer_ipv4`: currently a no-op; reserved for preferring IPv4 DNS results when both families are present.
...
- `http_proxy`: currently a no-op; reserved for an upstream proxy the server should use for outbound requests when relevant.
- `http_proxy_login`: currently a no-op; reserved for an optional proxy username override.
- `http_proxy_password`: currently a no-op; reserved for an optional proxy password override.
...
- `!ReverseTunnel` for reverse listeners (`ReverseTcp`, `ReverseUnix`, and related reverse tunnel protocol names)
