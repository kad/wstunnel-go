[ ] update README to match current implementation state and interoperability state :
...
[ ] Add support for running wstunnel-go via systemd
...
[ ] Add distribution packages for goreleaser target rules
[ ] Add support (scripts) for starting wstunnel-go client on Windows via task scheduler
